document.addEventListener('DOMContentLoaded', function() {
    ...
    const storage = firebase.storage(); // El almacén de archivos
            ...
            if (currentDayOfWeek !== 0 && currentDayOfWeek !== 6) {
                    ...
                    deleteFileBtn.onclick = async () => {
                        if (confirm(`¿Seguro que quieres eliminar el archivo "${fileData.name}"?`)) {
                            try {
                                ...
                                // Usar update con arrayRemove para quitar el archivo del array
                                await docRef.update({
                                    [`${dateKey}.files`]: firebase.firestore.FieldValue.arrayRemove(fileData)
                                });
                ...
                fileInput.onchange = async (e) => {
                    ...
                    try {
                        ...
                        // Corrección: Usar la notación de punto para añadir al array sin borrar otros datos del día.
                        const updateData = {};
                        updateData[`${dateKey}.files`] = firebase.firestore.FieldValue.arrayUnion(fileInfo);
    ...
    async function initApp() {
        // 1. CARGAR DATOS ESENCIALES (como la lista de usuarios)
        await loadUsers();

        // 2. PREPARAR LA INTERFAZ: Llenar años y establecer valores iniciales
        ...
        // 3. CONFIGURAR LOS BOTONES Y MENÚS (EVENT LISTENERS)
        ...
        calendarTable.addEventListener('blur', (event) => {
            ...
            if (target.classList.contains('cell-content')) {
                ...
                const updateData = {
                    [`${dateKey}.${field}`]: target.textContent,
                    [`${dateKey}.editor`]: currentUser
                };
        ...
        // 4. ARRANCAR LA APLICACIÓN: Empezar a escuchar los datos del mes actual
        listenToMonthData(currentYear, currentMonth);
    }
